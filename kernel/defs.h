...
void            proc_freepagetable(pagetable_t, uint64, struct proc* p);
int             kill(int);
...
void            syscall();
extern struct spinlock mmaplock;
...
pte_t *         walk_huge(pagetable_t, uint64, int);
void uvmunmap_huge(pagetable_t pagetable, uint64 va, uint64 npages, int do_free);
